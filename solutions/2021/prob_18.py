...
class CodeWarsHelper:
    ...
    @staticmethod
    def input_int_spaces_list(end="", separator=" ", extend=True) -> list:
        r = []
        for l in CodeWarsHelper.input_string_list(end):
            res = [int(i) for i in l.split(separator)]
            if extend:
                r.extend(res)
            else:
                r.append(res)
    ...
    @staticmethod
    def input_float_spaces_list(end="", separator=" ", extend=True) -> list:
        r = []
        for l in CodeWarsHelper.input_string_list(end):
            res = [float(i) for i in l.split(separator)]
            if extend:
                r.extend(res)
            else:
                r.append(res)
...
time, delay = CodeWarsHelper.input_int_spaces_tuple("+")

movements = CodeWarsHelper.input_float_spaces_list(end="#", extend=False)

remaining1, remaining2 = (float(time), float(time))

for move in movements:
    remaining1 -= move[0]
    remaining2 -= move[1]
    if move[0] != 0:
        remaining1 += int(delay)
    if move[1] != 0:
        remaining2 += int(delay)

print(f"{remaining1:.03f} {remaining2:.03f}")
