...
sentence = CodeWarsHelper.input_string().lower()
word = CodeWarsHelper.input_string()
...
if len(possibilities) == 0:
    print(word)
elif len(possibilities) == 1 or len(duplicated) == 1:
    print(possibilities[0])
else:
    equals = True
    before = None
    for x in duplicated:
        if before and x["count"] != before:
            equals = False
            break
        before = x["count"]
    if equals:
        print(CodeWarsHelper.get_list_alphabetic_order(possibilities)[0])
    else:
        # Get most common on duplicated and print it
        top = None
        topcount = None
        for x in duplicated:
            if topcount is None:
