...
for w in words:
  if w.isupper():
    final_words += UPPERSYM+UPPERSYM
    w = w.lower()
  for l in w:
    if l.isupper():
      final_words += UPPERSYM
      l = l.lower()
    final_words += braille_alphabet[l]
  final_words += "      "
  
lines = ["", "", ""]
for bchar in CodeWarsHelper.chunk_list(final_words, 6):
  for x in range(0, 5, 2):
    lines[x//2] += bchar[x] + bchar[x+1]
