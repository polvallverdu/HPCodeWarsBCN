...
columnsn, rowsn = CodeWarsHelper.input_int_spaces_tuple()
smallimg = int(input())
rawimg = CodeWarsHelper.input_string_list()

rows = [[] for _ in range(rowsn)]

for x, stringimg in enumerate(rawimg):
    rows[x%rowsn].append(stringimg)
