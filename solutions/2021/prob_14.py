...
columns, rows = CodeWarsHelper.input_int_spaces()
minutes = CodeWarsHelper.input_int()
raw = CodeWarsHelper.input_string_list("#")
...
for i in range(math.floor(minutes/10)):
  x = columns-(i+1)
  y = rows-(i+1)
  
  for s in list(m):
    if s[1] == i or s[1] == y or s[0] == i or s[0] == x:
      m.remove(s)
