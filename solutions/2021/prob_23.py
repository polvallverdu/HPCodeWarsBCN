...
class Item:
    ...
    def __init__(self, num, price, weight) -> None:
        self.num = num
        self.price = price
        self.weight = weight


prices = [int(i) for i in CodeWarsHelper.input_string().split(" ")]
weights = [float(i) for i in CodeWarsHelper.input_string().split(" ")]

items = []

for x in range(len(prices)):
    items.append(Item(x, prices[x], weights[x]))


# Removing weights more than 3
for i in list(items):
    if i.weight > 3:
        items.remove(i)
...
for item in items:
    for i in items:
        if i is item:
            continue
        if item.weight+i.weight <= 3:
            possibilities.append([item, i])

highest = []
highestNum = 0
for p in possibilities:
    if p[0].price + p[1].price > highestNum:
        highest = p
        highestNum = p[0].price + p[1].price

print("Mario chooses: prize {} and prize {} with a total profit of {} EUR".format(
    min(highest[0].num, highest[1].num), max(highest[0].num, highest[1].num), highestNum))
