...
def generateprimelist(num) -> list:
  primes = [1]
  for x in range(1, num+1):
    if CodeWarsHelper.is_prime(x):
      primes.append(x)
  return primes
...
startprime = CodeWarsHelper.is_prime(num) and CodeWarsHelper.is_prime(reverse)
if not startprime:
  print(f"Number {num} is not related to Sheldon prime")
  exit()

primes = generateprimelist(num)
primesrever = generateprimelist(reverse)

numpos = primes.index(num)
numposr = primesrever.index(reverse)

if numpos == int(str(numposr)[::-1]):
  print(f"Number {num} is a Sheldon prime!")
  exit()

if sorted(list(str(numpos))) == sorted(list(str(numposr))):
  print(f"Number {num} is a Sheldon prime relative")
  exit()

posprimes = generateprimelist(numpos)
posprimesreverse = generateprimelist(numposr)

if CodeWarsHelper.is_prime(numpos) and CodeWarsHelper.is_prime(numposr):
