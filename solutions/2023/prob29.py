...
pizzas = CodeWarsHelper.input_string_list(end="#")
...
for p in names_pizzas:
  valid_pizzas[p] = 0

for pizza in pizzas:
  if pizza in names_pizzas:
    valid_pizzas[pizza] += 1
    valid += 1
  else:
    wrong += 1
    wrong_pizzas.append(pizza)

ss = sorted(valid_pizzas, key=lambda x: x[1][])
print(f"Received valid pizza requests: {valid}")
for name, count in valid_pizzas.items():
  if count != 0:
    print("{}{}".format(name, f" {count}" if count > 1 else ""))
print("---")
print(f"Invalid requests: {wrong}")
for p in wrong_pizzas:
  print(p)